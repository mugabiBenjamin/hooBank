...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className="flex1 flex flex-col justify-start mr-10">
          ...
          <div key={footerlink.key} className="flex flex-col sm:my-0 my-4 min-w-[150px]">
            ...
            <ul className="list-none mt-4">
              {footerlink.links.map((link, index) => (
                <li key={link.name} className={`font-normal text-[16px] leading-[24px] hover:text-[#33bbcf] cursor-pointer text-white ${index !== footerlink.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                  {link.name}
                </li>
              ))}
            </ul>
